...
import 'package:talimger_mobile/models/category.dart';
import 'package:talimger_mobile/models/chart_model.dart';
import 'package:talimger_mobile/models/course.dart';
import 'package:talimger_mobile/models/lesson.dart';
import 'package:talimger_mobile/models/review.dart';
import 'package:talimger_mobile/models/section.dart';
import 'package:talimger_mobile/models/subscription.dart';
...
//TODO: make delete old image
  Future<String?> uploadImageToHosting(File image) async {
    String? imageUrl;
  ...
  Future<String?> uploadReceiptToHosting(File image) async {
    String? imageUrl;
    String imageName = image.path.split('/').last;
    final Reference storageReference =
        FirebaseStorage.instance.ref().child('receipts/$imageName');
    final UploadTask uploadTask = storageReference.putFile(image);
    await uploadTask.whenComplete(() async {
      imageUrl = await storageReference.getDownloadURL();
    });
  ...
  Future<bool?> isEnrollRequestExists(String userId, String courseId) async {
    try {
      ...
      log('Error checking enrollment request: $e');
      return null;
    }
  }

  Future<String?> createEnrollmentRequest(
      String userId, String courseId, String imageUrl, String authorId) async {
    try {
      final CollectionReference requestsCollection =
          firestore.collection('enrollment_requests');

      final DocumentReference newRequestDocRef = await requestsCollection.add({
        'user_id': userId,
        'course_id': courseId,
        'file_url': imageUrl,
