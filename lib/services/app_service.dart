...
class AppService {
  Future openLink(String url) async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      Fluttertoast.showToast(msg: "cant_launch_url".tr());
    }
  }
  ...
  Future openEmailSupport(String supportEmail) async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      openToast1("cant_open_email".tr());
    }
  }

  Future openReviewReportEmail(
      context, Review review, UserModel? user, String supportEmail) async {
  ...
  Future launchAppReview(context) async {
    await LaunchReview.launch(
        androidAppId: AppConfig.androidPackageName,
  ...
  static String getDateTime(DateTime dateTime, {Locale? locale}) {
    var format = DateFormat('dd MMMM, yyyy hh:mm a', locale?.languageCode);
    return format.format(dateTime.toLocal());
  }

  static String getDate(DateTime date, {Locale? locale}) {
    var format = DateFormat('dd MMMM yyyy', locale?.languageCode);
