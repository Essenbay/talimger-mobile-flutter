...
mixin SearchMixin {
  static void getRecentSearchList({required WidgetRef ref}) async {
    ...
    List<String> recentSearchData =
        sp.getStringList('recent_search_data') ?? [];
    ref
        .read(recentSearchDataProvider.notifier)
        .update((state) => [...recentSearchData]);
  }

  Future addToSearchList(
    ...
    ref
        .read(recentSearchDataProvider.notifier)
        .update((state) => [...searchData]);
  }

  Future removeFromSearchList(
      {required String value, required WidgetRef ref}) async {
    final SharedPreferences sp = await SharedPreferences.getInstance();
    List<String> searchData = ref.read(recentSearchDataProvider);
