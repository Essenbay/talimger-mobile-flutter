...
class _EditProfileState extends ConsumerState<EditProfile> {
  ...
  Future<String?> _getUserImage() async {
    if (_selectedImageFile != null) {
      final File image = File(_selectedImageFile!.path);
      final String? imageUrl =
          await FirebaseService().uploadImageToHosting(image);
      return imageUrl;
    } else {
  ...
  _handleUpdate() async {
    if (formKey.currentState!.validate()) {
      ...
      if (widget.user.imageUrl != imageUrl && widget.user.imageUrl != null) {
        await FirebaseService().deleteImage(widget.user.imageUrl!);
      }
