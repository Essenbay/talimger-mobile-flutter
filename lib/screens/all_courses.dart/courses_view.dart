...
import 'package:talimger_mobile/components/loading_grid_tile.dart';
...
import 'package:talimger_mobile/screens/all_courses.dart/grid_list_course_tile.dart';
  ...
  Future<QuerySnapshot> _getCourseQuery() async {
    ...
    if (widget.courseBy == CourseBy.latest) {
      snapshot = await FirebaseService()
          .getCoursesSnapshotByLatest(lastDocument: _lastDocument);
    } else if (widget.courseBy == CourseBy.category) {
      snapshot = await FirebaseService().getCoursesSnapshotByCategory(
          categoryId: widget.categoryId!, lastDocument: _lastDocument);
    } else if (widget.courseBy == CourseBy.free) {
      snapshot = await FirebaseService()
          .getCoursesSnapshotByFreeCourses(lastDocument: _lastDocument);
    } else if (widget.courseBy == CourseBy.tag) {
      snapshot = await FirebaseService().getCoursesSnapshotByTag(
          tagId: widget.tagId!, lastDocument: _lastDocument);
    } else if (widget.courseBy == CourseBy.author) {
      snapshot = await FirebaseService().getCoursesSnapshotByAuhtor(
