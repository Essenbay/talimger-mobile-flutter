...
final authorReviewsCountProvider =
    FutureProvider.family.autoDispose<int, String>((ref, authorId) async {
  final int count = await FirebaseService().getAuthorReviewsCount(authorId);
  return count;
});

final authorCoursesCountProvider =
    FutureProvider.family.autoDispose<int, String>((ref, authorId) async {
...
class AuthorProfile extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final int reviewsCount =
        ref.watch(authorReviewsCountProvider(user.id)).value ?? 0;
    final int courseCount =
        ref.watch(authorCoursesCountProvider(user.id)).value ?? 0;
